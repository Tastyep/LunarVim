...
M.config = function()
  lvim.builtin.terminal = {
    ...
    config = {
      ...
      -- { exec, keymap, name}
      -- lvim.builtin.terminal.config.execs = {{}} to overwrite
      -- table.insert(lvim.builtin.terminal.config.execs, {"gdb", "tg", "GNU Debugger"})
      execs = {
        { "lazygit", "gg", "LazyGit" },
      },
    },
...
M.setup = function()
  ...
  for _, exec in pairs(lvim.builtin.terminal.config.execs) do
    M.add_exec(exec[1], exec[2], exec[3])
  end
  -- NOTE: config.execs should not be passed to setup
